    ...
    <div className="space-y-8 pb-20 max-w-7xl mx-auto w-full overflow-x-hidden">
      ...
      <div className="bg-white dark:bg-[#0f172a] p-6 rounded-[2rem] border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col md:flex-row gap-6">
        <div className="flex-1">
          <label className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase mb-2 block tracking-widest">Filtrar Portfólio</label>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="bg-white dark:bg-[#0f172a]/60 border border-slate-200 dark:border-slate-800 rounded-[1.5rem] p-5 relative overflow-hidden shadow-sm group hover:border-emerald-500/30 transition-colors">
          ...
          <h3 className={`text-2xl font-black tracking-tighter ${totalProfit >= 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-red-600 dark:text-red-400'}`}>
        ...
        <div className="bg-white dark:bg-[#0f172a]/60 border border-slate-200 dark:border-slate-800 rounded-[1.5rem] p-5 relative overflow-hidden shadow-sm group hover:border-yellow-500/30 transition-colors">
          <p className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-1">Média por Dia Ativo</p>
          <h3 className="text-2xl font-black tracking-tighter text-slate-900 dark:text-white">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              return (
                <motion.button
                  ...
                  className={`aspect-square rounded-2xl border flex flex-col items-center justify-center relative transition-all
                    ${isSelected ? 'ring-4 ring-emerald-500/30 border-emerald-500 z-10' : ''}
                    ${!isSelected && !isDayToday ? 'border-slate-100 dark:border-slate-800 hover:border-slate-300 dark:hover:border-slate-600' : ''}
                    ${isDayToday && !isSelected ? 'border-blue-400 dark:border-blue-500' : ''}
                    ${hasData ? (isPositive ? 'bg-emerald-50 dark:bg-emerald-900/20' : 'bg-red-50 dark:bg-red-900/20') : 'bg-white dark:bg-[#020617]'}
                  `}
                >
        ...
        <div className="bg-white dark:bg-[#0f172a]/60 border border-slate-200 dark:border-slate-800 rounded-[2.5rem] p-8 shadow-sm flex flex-col h-[600px]">
          <CalendarIcon size={48} className="mb-4 text-emerald-500 opacity-20" />
          <div className="mb-6 border-b border-slate-200 dark:border-slate-800 pb-6">
            <h3 className="font-black text-xl text-slate-900 dark:text-white uppercase tracking-tighter italic">Relatório Diário</h3>
          ...
          <div className="space-y-3 overflow-y-auto custom-scrollbar flex-1 pr-2 scroll-smooth">
            {selectedDateBets.length === 0 ? (
              <div className="h-full flex flex-col items-center justify-center text-slate-400 dark:text-slate-600 opacity-50">
                <p className="text-[10px] font-black uppercase tracking-widest">Nenhuma operação consolidada</p>
              </div>
            ) : (
              selectedDateBets.map(bet => (
                <div key={bet.id} className="bg-slate-50 dark:bg-slate-900/50 rounded-2xl p-4 border border-slate-200 dark:border-slate-800 flex justify-between items-center group hover:border-slate-300 dark:hover:border-slate-700 transition-colors">
                  ...
                  <div className="text-right shrink-0">
                    <p className={`font-black font-mono text-sm tracking-tight ${bet.profit >= 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-red-600 dark:text-red-400'}`}>
                      {bet.profit > 0 ? '+' : ''} {formatValue(bet.profit)}
                    </p>
                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mt-0.5">Expo {formatValue(bet.stake)}</p>
                  </div>
                </div>
              ))
            )}
          ...
          {selectedDateBets.length > 0 && (
            <div className="mt-6 pt-6 border-t border-slate-200 dark:border-slate-800 flex justify-between items-center">
              <span className="text-slate-500 dark:text-slate-400 font-bold uppercase tracking-wider text-xs">Desempenho no Dia</span>

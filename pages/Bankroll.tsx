...
const Bankroll: React.FC = () => {
  ...
  const confirmDelete = async () => {
    if (!selectedBR || !isDeleteConfirmed) return;
  ...
  const cardClass =
    'bg-white dark:bg-[#1C1C1E] border border-slate-200 dark:border-[#2C2C2E] rounded-2xl p-6 shadow-sm';

  const inputClass =
    'bg-slate-50 dark:bg-[#000000] border border-slate-200 dark:border-[#3A3A3C] rounded-xl px-4 py-3 text-slate-900 dark:text-white font-bold text-sm outline-none focus:border-indigo-500 transition-colors w-full';
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-5">
        ...
        <div className={cardClass}>
          ...
          <p className="text-2xl font-bold text-slate-900 dark:text-white tracking-tight">
            {
              transactions.filter(
                (t) => t.bankrollId === activeBankrollId
              ).length
            }
          </p>
        </div>
        ...
        {bankrolls.map((br) => (
          ...
          >
            ...
            <div className="mt-4 pt-4 border-t border-slate-200 dark:border-[#2C2C2E]">
              <p className="text-[10px] text-slate-500 dark:text-[#8E8E93] font-bold uppercase tracking-widest mb-2">
                ...
                <div className="flex gap-3 w-full">
                  <div className="w-28 shrink-0 relative">
                    <select
                      value={newBRCurrency}
                      onChange={(e) =>
                        setNewBRCurrency(e.target.value)
                      }
                      className={`${inputClass} appearance-none cursor-pointer pr-10`}
                    >
                  ...
                  <div className="flex-1 min-w-0">
                    <input
                      ...
                      value={newBRBalance}
                      onChange={(e) =>
                        setNewBRBalance(e.target.value)
                      }
                      className={inputClass}
                    />
                  </div>
          ...
          >
            ...
            >
              ...
              <p className="text-sm mb-2 text-slate-600 dark:text-[#E5E5EA] font-medium leading-relaxed">
                Você está prestes a liquidar e excluir permanentemente o
                portfólio:
              </p>
              ...
              <label className="flex items-start gap-3 cursor-pointer mb-8 p-4 border border-red-200 dark:border-red-500/30 rounded-xl bg-red-50 dark:bg-red-500/10 transition-colors">
                ...
                <span className="text-xs font-medium text-red-800 dark:text-red-300 select-none leading-relaxed">
                  Sim, entendo que vou perder todo o histórico de
                  operações, fluxo de caixa e relatórios vinculados a este
                  portfólio. Ação irreversível.
                </span>
              </label>

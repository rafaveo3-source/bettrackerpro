...
const Calculators: React.FC = () => {
  ...
  return (
    <div className="space-y-6 pb-20 w-full overflow-hidden px-1 md:px-0">
       <div className="flex items-center gap-3">
        ...
        <div className="min-w-0">
            <h1 className="text-2xl font-black text-slate-900 dark:text-white uppercase tracking-tighter italic truncate">Ferramentas Pro</h1>
            <p className="text-slate-500 dark:text-slate-400 text-xs font-bold uppercase tracking-widest mt-1 truncate">Gestão Matemática & Estratégia</p>
        </div>
      </div>

      {/* Tabs com Scroll Horizontal Seguro */}
      <div className="w-full overflow-x-auto pb-2 scrollbar-hide -mx-1 px-1">
        <div className="flex gap-3 min-w-max">
            <button onClick={() => setActiveTab('dutching')} className={`px-6 py-3 rounded-xl font-black text-xs uppercase tracking-widest transition-all ${activeTab === 'dutching' ? 'bg-emerald-500 text-white shadow-lg shadow-emerald-500/20 scale-105' : 'bg-white dark:bg-[#0f172a] text-slate-500 border border-slate-200 dark:border-slate-800'}`}>Dutching</button>
            <button onClick={() => setActiveTab('kelly')} className={`px-6 py-3 rounded-xl font-black text-xs uppercase tracking-widest transition-all ${activeTab === 'kelly' ? 'bg-emerald-500 text-white shadow-lg shadow-emerald-500/20 scale-105' : 'bg-white dark:bg-[#0f172a] text-slate-500 border border-slate-200 dark:border-slate-800'}`}>Critério de Kelly</button>
            <button onClick={() => setActiveTab('converter')} className={`px-6 py-3 rounded-xl font-black text-xs uppercase tracking-widest transition-all ${activeTab === 'converter' ? 'bg-emerald-500 text-white shadow-lg shadow-emerald-500/20 scale-105' : 'bg-white dark:bg-[#0f172a] text-slate-500 border border-slate-200 dark:border-slate-800'}`}>Conversor Odds</button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-6 min-w-0 w-full">
            ...
            {activeTab === 'dutching' && (
                <div className="bg-white dark:bg-[#0f172a] border border-slate-200 dark:border-slate-800 rounded-[1.5rem] md:rounded-[2rem] p-4 md:p-8 shadow-sm overflow-hidden w-full">
                    {/* ACCORDION INFO */}
                    ...
                    <h2 className="text-xl font-black text-slate-900 dark:text-white uppercase tracking-tighter italic mb-8">Conversor de Odds</h2>
                      <div className="space-y-6">
                        <div>
                            ...
                            <input type="number" value={decimalOdd} onChange={e => setDecimalOdd(e.target.value)} className="w-full bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-4 text-slate-900 dark:text-white font-mono font-bold text-lg focus:border-purple-500 outline-none" />
                        </div>

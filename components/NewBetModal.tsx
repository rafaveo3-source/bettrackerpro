...
const NewBetModal: React.FC<NewBetModalProps> = ({
  ...
  // Atualiza o formulário ao abrir ou editar
  useEffect(() => {
    if (betToEdit) {
      setFormData({
      ...
      // Se o mercado não estiver na lista padrão, assume que é manual
      const isCustomMarket = !customMarkets.some(m => m.name === betToEdit.market);
      if (isCustomMarket && betToEdit.market) {
         // O formulário usa o valor do mercado diretamente, mas precisamos setar o manualMarket
         // e o select para __custom para edição correta se desejar
      }
    } else {
      setFormData({
        sport: 'Futebol',
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Se estiver em modo units, converte para valor monetário para salvar no banco
  ...
  const calculateResult = () => {
    ...
    // Normaliza stake para cálculo visual se estiver em units
    if (displayMode === 'units') {
    ...
    switch (formData.status) {
      ...
      case 'refunded':
        return 0;
      default:
        // Se pendente, mostra o potencial
        return stake * odds - stake;
    }
  };
  ...
  const inputStyle =
    'w-full bg-slate-950 border border-slate-700 text-slate-200 rounded-xl px-4 py-3 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/30 outline-none transition-all placeholder:text-slate-600';
  ...
  const statusOptions = [
    ...
    { id: 'void', label: 'Anulada', icon: Ban, color: 'bg-slate-500 text-white', activeRing: 'ring-slate-400' },
  ];
            ...
            <form onSubmit={handleSubmit} className="p-6 md:p-8 space-y-6 md:space-y-8 overflow-y-auto max-h-[85vh] md:max-h-none scrollbar-hide">
              ...
              <div className="grid grid-cols-1 md:grid-cols-12 gap-5">
                ...
                <div className="md:col-span-4 space-y-2">
                  <label className="text-xs uppercase text-slate-500 font-semibold tracking-wider ml-1">Esporte</label>
                  <select
                ...
                <div className="md:col-span-8 space-y-2">
                  <label className="text-xs uppercase text-slate-500 font-semibold tracking-wider ml-1">Evento</label>
                  <input
                    type="text"
                ...
                {/* LINHA 3: Metadados (Método, Estratégia) */}
                <div className="md:col-span-6 space-y-2">

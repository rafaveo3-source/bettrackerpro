    ...
    <>
      ...
      >
        <div className="h-full flex flex-col">
          ...
          <div className="p-4 space-y-2 border-t border-slate-200 dark:border-white/10">
            {/* ✅ Botão de Tema */}
            <button 
                onClick={toggleTheme}
                className="w-full flex items-center justify-center gap-2 rounded-xl px-3 py-2.5 text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-900 transition-colors"
            >
                {isDarkMode ? <Sun size={16} /> : <Moon size={16} />}
                <span className="text-sm font-semibold">{isDarkMode ? 'Modo Claro' : 'Modo Escuro'}</span>
            </button>

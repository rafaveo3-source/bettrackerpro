...
const Dashboard: React.FC = () => {
  const { currentBankrollBalance, bankrolls, activeBankrollId, getMetrics, history, isDarkMode, removeBet, primaryColor } = useBetStore();
  ...
  // Determine chart color based on store or default to Gold (#f59e0b)
  const chartColor = primaryColor === 'emerald' ? '#10b981' : 
                     primaryColor === 'blue' ? '#3b82f6' : 
                     primaryColor === 'purple' ? '#a855f7' : 
                     primaryColor === 'orange' ? '#f97316' : '#f59e0b';
            ...
            <div className="h-[400px]">
                <ResponsiveContainer width="100%" height="100%">
                    <AreaChart data={chartData}>
                        ...
                        <Tooltip contentStyle={{ background: isDarkMode ? '#18181b' : '#fff', border: isDarkMode ? '1px solid #27272a' : '1px solid #e4e4e7', borderRadius: '1.5rem', boxShadow: '0 20px 25px -5px rgba(0,0,0,0.3)', color: isDarkMode ? '#fff' : '#000' }} />
                        <Area type="monotone" dataKey="balance" stroke={chartColor} strokeWidth={5} fill="url(#colorBal)" />
                    </AreaChart>
                </ResponsiveContainer>
            </div>
        </div>

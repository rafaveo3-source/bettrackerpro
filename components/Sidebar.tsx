...
export const Sidebar: React.FC<SidebarProps> = ({ currentView, setView }) => {
  ...
  return (
    <>
      ...
      {/* Sidebar Container */}
      <aside className={`
        fixed top-0 left-0 h-full bg-[#020617] border-r border-slate-800 z-40 transition-transform duration-300 w-64 flex flex-col
      ...
      `}>
        ...
        <div className="p-6 border-b border-slate-800/50">
            <div className="flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                    <p className="text-sm font-bold text-white truncate">{user?.name || 'Apostador'}</p>
            ...
            {menuItems.map(item => {
                ...
                return (
                    <button
                        ...
                        `}
                    >
                        <item.icon size={18} className={isActive ? 'text-emerald-500' : 'text-slate-500 group-hover:text-white'} />
                        {item.label}
                    </button>
                )
            })}
        ...
        <div className="p-4 border-t border-slate-800/50 bg-[#020617]">
            {!isPro && (
                <div className="mb-4 p-4 rounded-2xl bg-gradient-to-br from-slate-900 to-slate-950 border border-slate-800 text-center relative overflow-hidden group cursor-pointer" onClick={() => handleNavigation('pro')}>
                    <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-emerald-500 to-cyan-500" />
                    <Crown size={24} className="text-emerald-500 mx-auto mb-2" />
                    <h4 className="text-white font-bold text-xs uppercase tracking-widest mb-1">Seja Profissional</h4>
                    <p className="text-[10px] text-slate-400 mb-3">Libere todas as ferramentas</p>
                    <button 
                        onClick={(e) => {
                            e.stopPropagation();

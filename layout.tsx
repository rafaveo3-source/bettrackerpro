...
const Layout: React.FC<LayoutProps> = ({ children, currentView, setView }) => {
  ...
  const tutorialSteps: Step[] = useMemo(() => [
    { target: 'body', placement: 'center', title: <><Rocket className="text-emerald-500" /> Start do Sistema</>, content: 'Bem-vindo ao BetTracker PRO. Esqueça planilhas amadoras. Vamos blindar o seu capital e escalar sua consistência no mercado esportivo.', disableBeacon: true },
    { target: '.tour-sidebar-dashboard', placement: 'right', title: <><BarChart3 className="text-blue-500" /> Cockpit de Análise</>, content: 'Onde o dinheiro é medido. Aqui você acompanha sua Win Rate real, descobre o EV+ das suas operações e mede seu Max Drawdown para evitar quebras.' },
    { target: '.tour-sidebar-scout', placement: 'right', title: <><Sparkles className="text-indigo-500" /> Motor Scout IA (PRO)</>, content: 'A joia da coroa. Nossa Inteligência Artificial lê suas estatísticas, calcula covariância em tempo real e monta bilhetes com Odd Justa matemática. Um cheat code legalizado.' },
    { target: '.tour-sidebar-bankroll', placement: 'right', title: <><Wallet className="text-amber-500" /> Gestão de Caixas</>, content: 'Nunca misture o dinheiro. Crie múltiplos Portfólios para separar sua banca de cantos, gestão conservadora, ou caixa alavancado de alta exposição.' },
    { target: '.tour-sidebar-metas', placement: 'right', title: <><Target className="text-purple-500" /> Sistema Take Profit</>, content: 'O mercado não tem fim, mas sua meta deve ter. Configure alvos e o sistema travará a banca automaticamente quando você atingir o lucro desejado.' },
    { target: '.tour-sidebar-mindset', placement: 'right', title: <><BrainCircuit className="text-orange-500" /> Módulo Psicológico</>, content: 'A maioria quebra por descontrole emocional (Tilt). Registre como você se sentiu e veja como sua mente impacta o seu ROI a longo prazo.' },
    { target: '.tour-sidebar-calendar', placement: 'right', title: <><CalendarDays className="text-pink-500" /> Calendário Financeiro</>, content: 'Acompanhe seus dias de Green e Red de forma visual. O calendário consolida o P&L diário para você entender seus ciclos de lucro.' },
    { target: '.tour-sidebar-calculators', placement: 'right', title: <><Calculator className="text-teal-500" /> Calculadoras PRO</>, content: 'Valide suas entradas antes de apostar. Ferramentas matemáticas como EV+, Critério de Kelly e Arbitragem garantem precisão técnica.' },
    { target: '.tour-sidebar-biblioteca', placement: 'right', title: <><BookOpen className="text-indigo-500" /> Playbooks e Gestão</>, content: 'Acesse métodos validados por profissionais. Descubra novas formas de operar e copie blueprints de progressão direto para o seu perfil.' },
    { target: '.tour-fab-button', placement: 'top-end', title: <><ShieldAlert className="text-emerald-500" /> Registre o Green</>, content: 'Acabou a operação? Clique aqui para cadastrar a Stake e a Odd. O motor do BetTracker fará todo o cruzamento de dados matemáticos.' }
  ], []);
  ...
  // 🔥 FIX: A Coleira de Segurança do Joyride
  // O Tutorial só pode renderizar SE o usuário estiver na tela inicial (Dashboard).
  // Assim que ele clica em "Analytics" (por exemplo), currentView vira 'analytics',
  // o Joyride é destruído instantaneamente e o React Router consegue carregar a página livre de bugs!
